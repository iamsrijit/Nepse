...
Original file is located at
    https://colab.research.google.com/drive/1AQeG54UbybG1xjzOSzGYoRghIFHJ0rXV
"""
...
def process_symbol(symbol, group):
    ...
    buy_crossover = (group['EMA_20'] > group['EMA_50']) & (group['EMA_20'].shift(1) <= group['EMA_50'].shift(1))

    # Detect sell crossover
    sell_crossover = (group['EMA_20'] < group['EMA_50']) & (group['EMA_20'].shift(1) >= group['EMA_50'].shift(1))

    # Conditions for buy signal (entry)
    buy_conditions = buy_crossover & (group['RSI'].between(30, 70)) & (group['Close'] > group['Close'].rolling(window=60, min_periods=1).max().shift(1) * 0.95)

    # Conditions for sell signal (exit)
    sell_conditions = sell_crossover & (~group['RSI'].between(30, 70)) & (group['Close'] < group['Close'].rolling(window=60, min_periods=1).min().shift(1) * 1.05)

    group['Buy_Signal'] = buy_conditions
    group['Sell_Signal'] = sell_conditions

    # Filter rows with signals
    valid_signals = group[group['Buy_Signal'] | group['Sell_Signal']]
...
all_valid_signals = pd.concat(results).reset_index(drop=True)

# Display valid signal dates
# print("\nValid EMA Signal Dates for All Symbols:")
# print(all_valid_signals[['Symbol', 'Date', 'Close', 'EMA_20', 'EMA_50', 'RSI', 'Buy_Signal', 'Sell_Signal']])

# Save the valid signals to a CSV file
all_valid_signals.to_csv('valid_ema_signals.csv', index=False)
# print("\nValid signals saved to 'valid_ema_signals.csv'.")

# Output only the latest dates in valid EMA Signal Dates for each symbol
combinedd_df = all_valid_signals.sort_values('Date', ascending=False).drop_duplicates('Symbol')
combinedd_df['Signal'] = np.where(combinedd_df['Buy_Signal'], 'Buy', np.where(combinedd_df['Sell_Signal'], 'Sell', ''))
print("\nLatest Valid EMA Signal Dates for Each Symbol:")
print(combinedd_df[['Symbol', 'Date', 'Close', 'Signal']])
#  'EMA_20', 'EMA_50', 'RSI', 'Volume', '30D_Avg_Volume']])

# Save the latest valid signals to a CSV file
combinedd_df.to_csv('latest_valid_ema_signals.csv', index=False)
print("\nLatest valid signals saved to 'latest_valid_ema_signals.csv'.")
