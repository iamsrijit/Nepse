...
# RSI settings — focus on oversold only (for buy signals, matching your cheap stock focus)
RSI_PERIOD = 14
RSI_OVERSOLD = 30  # Only include ≤ this for potential buys; removed overbought
...
def delete_old_rsi_files(prefix, keep_filename):
    ...
    for f in r.json():
        ...
        if name.startswith(prefix) and name.endswith(".csv") and name != keep_filename:
            ...
            if res.status_code == 200:
                ...
                print(f"⚠️ Delete failed for {name}")
...
for sym in df_market["Symbol"].unique():
    ...
    # Only oversold signals (potential buys)
...
if signals:
    ...
    # ─── Upload to GitHub ────────────────────────────────────────────────
    latest_date = df_market['Date'].max().strftime("%Y-%m-%d")
    filename = f"RSI_SIGNALS_{latest_date}.csv"
    
    upload_to_github(filename, df_signals.to_csv(index=False))
    
    # ─── Clean up old files ──────────────────────────────────────────────
    delete_old_rsi_files("RSI_SIGNALS_", filename)
    
else:
    print("No strong RSI signals (oversold) found.")
